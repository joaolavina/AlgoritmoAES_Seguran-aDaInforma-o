public static class Util
{
    ...
    public static byte[,] AddRoundKey(byte[,] matrizEstado, byte[][] keySchedule, int rodada)
    {
        ...
        for (int col = 0; col < 4; col++)
        {
            byte[] palavra = keySchedule[rodada * 4 + col];

            for (int lin = 0; lin < 4; lin++)
                resultado[lin, col] = (byte)(matrizEstado[lin, col] ^ palavra[lin]);
        }

        return resultado;
    }

    public static byte MultiplicarGalois(byte a, byte b)
    {
        if (a == 0 || b == 0) return 0;
        if (a == 1) return b;
        if (b == 1) return a;

        int soma = TabelaL[a] + TabelaL[b];

        if (soma > 0xFF) soma -= 0xFF;

        return TabelaE[soma];
    }

    private static readonly byte[] TabelaL =
    [
    ...
    private static readonly byte[] TabelaE =
    [
